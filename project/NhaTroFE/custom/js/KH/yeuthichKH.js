...
// js danh sach nha tro tu yeu thich
...
function fetchNhaTro() {
  ...
  const apiUrl = `http://localhost:8080/yeu-thich/${idUser}`; // Đường dẫn đến API của bạn
      ...
      nhaTro.tieuDe
                      ...
                      <div class="card-body">
                          ...
                          <a href="yeuthichKH.html" class="btn btn-danger w-100" data-id="${
                            nhaTro.id
                          }" onclick="removeFromFavorites(event)">Bỏ yêu thích</a>
                      </div>
                  </div>
              </div>`;
...
function removeFromFavorites(event) {
  ...
  const apiUrl = `http://localhost:8080/yeu-thich`;
  ...
  fetch(apiUrl, {
    ...
    .then((response) => {
      ...
      alert("Xóa khỏi yêu thích thành công");
      fetchNhaTro();
      return response.json();
    })
    .then((data) => {
