...
let currentPage = 1;
const itemsPerPage = 4; // Số nhà trọ hiển thị trên mỗi trang
let totalItems = 0;
...
function fetchNhaTro() {
  const apiUrl = "http://localhost:8080/nha-tro/khach/show";
...
// Thêm hàm mới để cuộn đến vị trí danh sách nhà trọ
function scrollToNhaTro() {
  const nhaTroSection = document.querySelector("#ds-tro");
  if (nhaTroSection) {
    nhaTroSection.scrollIntoView({ 
      behavior: 'smooth', 
      block: 'start'
    });
  }
